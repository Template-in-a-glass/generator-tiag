# Init CDK project
mkdir $APP_NAME-landing-page-ui-library-infra
cd $APP_NAME-landing-page-ui-library-infra
...
curl https://codeload.github.com/Template-in-a-glass/template-landing-page-ui-library-infra/tar.gz/main | tar -xz --strip=1 template-landing-page-ui-library-infra-main/src/
...
gh repo create $GH_ORGA/$APP_NAME-landing-page-ui-library-infra --private
git remote add origin git@github.com:$GH_ORGA/$APP_NAME-landing-page-ui-library-infra.git
git branch -M main
git push -u origin main

# Deploy infra
npm pkg set scripts.deploy="npx -y cdk@latest deploy -c domain=$DOMAIN_NAME -c subdomain=$APP_NAME-landing-page-ui-library -c stackName=$APP_NAME --require-approval never"
